...
def mainPage(tt):
	if tt == None:
		...
		tt = 'https://www.imdb.com/title/' + str(tt)
...
def castPage(tt):
	if tt == None:
		...
		tt = 'https://www.imdb.com/title/' + str(tt) + '/fullcredits'
...
def numAllresults(start, end):
	from bs4 import BeautifulSoup

	import requests, re

	url = 'https://www.imdb.com/search/title?&release_date=' + str(start) + ',' + str(end) + '&countries=us&adult=include&sort=release_date,asc&count=250&start=' + str(1)
